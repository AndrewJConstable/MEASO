...
# functions
##############################################
kmLongitude<- # calculate km per degree longitude, given latitude
  function(Lat){111.12*cos(Lat*pi/180)}  # km of one degree longitude at equator is 60/0.539957=111.12 km

##############################################
cellMode <- # calculate value that has the greatest weight in a cell.  
#      allows for 
#      1. multiple observations of the same value
#      2. ties for the greatest weight (randomly chooses between them) (slightly biased formulation)
    function(x) {  
...
##############################################
fnCellVal<- # determines the value of the cell (polygon) from a raster
  function(LL       # vector of lon, lat
          ,rasterPR # raster
          ,kmRes    # resolution of the square cell - distance of one side
          ){  # begin function
    poly<-polyCell(LL["Lon"],LL["Lat"],kmRes)  # create polygon of square cell
    if(length(poly)>1) {  # take note if cell crosses 180/-180 boundary
...
MEASO_space<-Seabed_areas

remove(Seabed_areas)  # free up memory

```

## load pelagic regionalisation

```{r, echo=FALSE}

#subset Cbed
UseMEASO_space<-!is.na(MEASO_space[,"MEASO"]) & MEASO_space[,"Lat"] <= (-40)

# PR from CSV file (coarser resolution)
...
# res1<-apply(Cbed_cell[c(1:1E4),],1,fnCellVal,rasterPR,2)
res1<-extract(rasterPR,MEASO_space[UseMEASO_space,c("Lon","Lat")],small=TRUE)

MEASO_space<-bind_cols(MEASO_space,data.frame(Pelagic_Reg = rep(NA,nrow(MEASO_space))))
MEASO_space[UseMEASO_space,"Pelagic_Reg"]<-res1

# Map grid and projection
prj <- "+proj=laea +lat_0=-90 +datum=WGS84"
r <- raster(extent(-5398000, 5386000, -5260000, 5570000)
                   , nrows = 5415, ncols = 5392, crs = prj)

r[MEASO_space[UseMEASO_space,"Cell"]] <- res1
...
# BR from CSV file (coarser resolution)


BR<-rgdal::readOGR("SOBC/SO_Benthic_Classification_v2_0.shp")
MEASOpts<-MEASO_space[UseMEASO_space,c("Lon","Lat")]
coordinates(MEASOpts)<- ~Lon+Lat
projection(MEASOpts) <- "+proj=longlat"
MEASOpts2 <- spTransform(MEASOpts, CRSobj = projection(BR)) ## projection of BR is laea thus reproject points to laea
resBR<-over(MEASOpts2,BR)


MEASO_space<-bind_cols(MEASO_space[UseMEASO_space,],resBR)

saveRDS(MEASO_space,"MEASO_space_PR_BR")
```
...
RAATDp<-raster("RAATD-projections/RAATD_AES.grd")
plot(RAATDp, ylim=c(-80,-40))
...
r[MEASO_space[,"Cell"]] <- MEASO_space$
